...
| Border Export Permit Voucher Report | `dbo.sp_VoucherReport_pagination` + ~~`dbo.sp_ExportPermitVoucherCurrencyTotals`~~ | DB data-show + footer totals passed | 106 ms one-day page / 40 ms one-day footer / 107 ms broad page / 45 ms broad footer | Old RDLC footer sums voucher `Amount`; patched Border Export Permit voucher date filters to include the full selected ToDate and kept footer totals aligned. **CORRECTION 2026-09-04: the footer was NOT aligned.** The config's `valueColumnKey` pointed at `'Amount'` (right heading) but `sp_ExportPermitVoucherCurrencyTotals.sql:42` sums `BorderExportPermitItem.Amount` - the goods value - so a foreign-currency total printed under the MMK fee column. Harder to spot than the Border Export Licence sibling, and not a safe template. Now replaced by the legacy single `SUM(Amount)` `ColumnTotals["amount"]` footer. |
| Border Export Licence Voucher Report (2026-09-04 complaint) | `dbo.sp_VoucherReport_pagination` + `sp_VoucherReport.ExecuteAmountTotalAsync` | Frontend config + backend contract suites pass; DB-free | n/a (no proc change) | Customer searched EIR `128709231`: row count matched Tradenet 2.0 but the total showed `CNY:4,305,000.0000` vs the old `100,000`. Cause: `sp_VoucherReport` returns two colliding money columns - `Amount` = `AccountTransaction.TotalAmount` (MMK fee) and `TotalAmount` = `SUM(<Doc>Item.Amount)` (goods value) - and commit `43f9eeb` bound the report's only money column AND its footer to the second, while `BorderVoucherReport.rdlc:1399/1521` binds and totals the first. Fix: money column -> `amount`; the per-currency footer replaced by the legacy single `TOTAL` (`ColumnTotals["amount"]`, rounded to 0 dp = the rdlc's `N0`); the `ApplyType='New'` duplicate Licence No column now hidden (rdlc:1578). Same defect swept on Border Import Permit Voucher (identical wrong column), Border Export Permit Voucher and Export Permit Voucher (wrong footer number under a right heading). `sp_Export{Licence,Permit}VoucherCurrencyTotals` are now unused - annotated, not dropped. Still owed family-wide: the legacy `TOTAL` row on Border Import Licence Voucher, Export Licence Voucher and Import Permit Voucher, and a decision on the invented per-currency `Lic Value` footer on Import Licence / Import Permit Voucher (neither RDLC totals Lic Value). Also still owed on Border Import Permit Voucher only: it has no `resolveColumns`, so its `ParametersHeader2Value` / `ParametersHeader3Value` columns never retitle per `ApplyType` (the old rdlc does, ReportsController.cs:14269+) and its header2 column is never hidden for `ApplyType='New'`; adding the shared resolver there is not a drop-in because its column 2 is `LicenceNo -> licenceNo` rather than `OriginalLicenceNo -> oldLicenceNo`, which would collide with the resolver's hide rule. |
