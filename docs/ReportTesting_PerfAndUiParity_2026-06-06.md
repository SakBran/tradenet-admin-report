...
| BorderExportLicenceVoucherReport | MAJOR | • **CORRECTED 2026-09-04 — the column claim below was wrong.** Re-verified against the RDLC XML: the old report has 11 columns, does NOT have 'Application Date' or 'Commodity Type', and the new config does NOT have 'Lic Value'/'Currency'/'Approved User'. Real diff: need_in_new=0, extra_in_new=2 ('Application Date', 'Commodity Type' - kept deliberately, consistent with the sibling border voucher reports). See docs/ReportColumnComparison.md for the verified lists. Original (wrong) finding: "new drops old columns 'Application Date', 'Commodity Type' and the two dynamic header columns while adding 'Licence Date', 'Lic Value', 'Currency', 'Approved User'".<br>• ApplyType option set differs: old Border Export Licence Voucher action builds ApplyTypeList = GetApplyTypeList() minus 'Fine' = New, Amend, Extension, Cancel, Actual Amend (NO De-Cancel and no 'All' prompt). New config adds an extra 'De-Cancel' option (6 options).<br>• PaymentType option set differs: old PaymentTypeList is DB-driven (paymentTypeRepository.GetAll() where IsActive, by Id/Name) whereas new hardcodes All / MPU / CitizenPay - values may not match the live DB payment-type rows.<br>• **Total row: WRONG, every clause — and this false "(match)" is why the report was skipped until a customer complained.** `BorderVoucherReport.rdlc` DOES have a grand-total footer: one static row, `TOTAL` at :1457 (ColSpan 10) + `=FORMAT(SUM(Fields!Amount.Value),"N0")` at :1521. And the new controller DID set a footer - `CurrencyTotals` from `sp_ExportLicenceVoucherCurrencyTotals`, which sums `BorderExportLicenceItem.Amount` (the goods value) and printed `CNY:4,305,000.0000` where the old system printed `100,000`. Fixed 2026-09-04: the column and the footer now bind `AccountTransaction.TotalAmount` via `ColumnTotals["amount"]`.<br>• The `SumFieldsCount` probe that produced this row demonstrably misses real RDLC footers, so **every** "no Sum footer -> (match)" verdict in this document must be re-confirmed by grepping the RDLC for `SUM(Fields!` before it is trusted (`BorderExportLicenceDetailReport` and `BorderExportPermitDetailReport` carry the same claim from the same probe). |
