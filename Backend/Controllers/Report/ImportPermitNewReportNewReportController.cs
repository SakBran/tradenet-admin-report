...
namespace Backend.Controllers.Report
{
    ...
    public class ImportPermitNewReportNewReportController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "ImportPermitNewReportNewReport";
        ...
        public ImportPermitNewReportNewReportController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        [HttpPost]
        public async Task<ActionResult<ApiResult<sp_NewReportResult>>> Post([FromBody] ImportPermitNewReportNewReportRequest? request)
        {
            ...
            if (data.Count > 0)
            {
                result.CurrencyTotals = await ImportPermitListingCurrencyTotals.ExecuteAsync(
                    ...
                    procedureRequest.ExportImportSectionId, procedureRequest.CompanyRegistrationNo, 0);
            }
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] ImportPermitNewReportNewReportRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Import Permit New Report (New Report )";
        public Type ExcelRequestType => typeof(ImportPermitNewReportNewReportRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((ImportPermitNewReportNewReportRequest)request, sink, chunkSize, cancellationToken);

        private async Task WriteRowsAsync(
            ImportPermitNewReportNewReportRequest request,
            IExcelRowSink sink,
            int chunkSize,
    ...
    public sealed class ImportPermitNewReportNewReportRequest : ReportQueryRequest
    {
