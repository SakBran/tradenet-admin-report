...
namespace Backend.Controllers.Report
{
    ...
    public class ImportLicenceActualAmendmentReportController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "ImportLicenceActualAmendmentReport";
        ...
        public ImportLicenceActualAmendmentReportController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        [HttpPost]
        public async Task<ActionResult<ApiResult<sp_ActualAmendReportResult>>> Post([FromBody] ImportLicenceActualAmendmentReportRequest? request)
        {
            ...
            var data = rows.Select(r => r.ToResult()).ToList();
            ...
            if (data.Count > 0)
            {
                result.CurrencyTotals = await ImportLicenceListingCurrencyTotals.ExecuteAsync(
                    ...
                    procedureRequest.AmendRemarkId);
            }
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] ImportLicenceActualAmendmentReportRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Import Licence Actual Amendment Report";
        public Type ExcelRequestType => typeof(ImportLicenceActualAmendmentReportRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((ImportLicenceActualAmendmentReportRequest)request, sink, chunkSize, cancellationToken);

        private async Task WriteRowsAsync(
            ImportLicenceActualAmendmentReportRequest request,
            IExcelRowSink sink,
            int chunkSize,
            CancellationToken cancellationToken)
        {
            ...
            {
                sink.Append(chunk.Select(r => r.ToResult()).ToList());
            }
        }
    ...
    public sealed class ImportLicenceActualAmendmentReportRequest : ReportQueryRequest
    {
