...
namespace Backend.Controllers.Report
{
    ...
    public class ImportLicenceVoucherReportController : ControllerBase
    {
        ...
        public async Task<IActionResult> Excel([FromBody] ImportLicenceVoucherReportRequest? request)
        {
            ...
            var data = rows.Select(row => row.ToResult()).ToList();
            var fileBytes = ExcelGenerator.CreateWorkbook(data, "Import Licence Voucher Report");

            return File(fileBytes, ExcelGenerator.ContentType, "ImportLicenceVoucherReport.xlsx");
        }
