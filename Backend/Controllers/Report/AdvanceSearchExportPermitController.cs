...
{
    /// <summary>
    /// Advance Search for Export Permit -- the old admin's Advance Search screen scoped to <c>Export Permit</c>
    ...
    public class AdvanceSearchExportPermitController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "AdvanceSearchExportPermit";
        private const string LicenceOrPermitType = sp_AdvanceSearch.AdvanceSearchType.ExportPermit;

        private readonly TradeNetDbContext _context;
        private readonly IExcelExportJobService _excelExportJobs;

        public AdvanceSearchExportPermitController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        public async Task<ActionResult<ApiResult<sp_AdvanceSearchResult>>> Post(
            [FromBody] AdvanceSearchExportPermitRequest? request)
        {
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] AdvanceSearchExportPermitRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Advance Search for Export Permit";
        public Type ExcelRequestType => typeof(AdvanceSearchExportPermitRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((AdvanceSearchExportPermitRequest)request, sink, chunkSize, cancellationToken);

        private Task WriteRowsAsync(
            AdvanceSearchExportPermitRequest request,
            IExcelRowSink sink,
            int chunkSize,
    ...
    public sealed class AdvanceSearchExportPermitRequest : ReportQueryRequest
    {
