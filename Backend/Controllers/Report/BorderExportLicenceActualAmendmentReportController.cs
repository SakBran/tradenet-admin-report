...
namespace Backend.Controllers.Report
{
    ...
    public class BorderExportLicenceActualAmendmentReportController : ControllerBase
    {
        ...
        public async Task<IActionResult> Excel([FromBody] BorderExportLicenceActualAmendmentReportRequest? request)
        {
            ...
            var data = rows.Select(row => row.ToResult()).ToList();
            var fileBytes = ExcelGenerator.CreateWorkbook(data, "Border Export Licence Actual Amendment Report");

            return File(fileBytes, ExcelGenerator.ContentType, "BorderExportLicenceActualAmendmentReport.xlsx");
        }
