...
namespace Backend.Controllers.Report
{
    ...
    public class ImportPermitActualAmendmentReportController : ControllerBase
    {
        ...
        public async Task<IActionResult> Excel([FromBody] ImportPermitActualAmendmentReportRequest? request)
        {
            ...
            var data = rows.Select(row => row.ToResult()).ToList();
            var fileBytes = ExcelGenerator.CreateWorkbook(data, "Import Permit Actual Amendment Report");

            return File(fileBytes, ExcelGenerator.ContentType, "ImportPermitActualAmendmentReport.xlsx");
        }
