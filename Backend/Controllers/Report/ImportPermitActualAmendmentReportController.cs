...
namespace Backend.Controllers.Report
{
    ...
    public class ImportPermitActualAmendmentReportController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "ImportPermitActualAmendmentReport";
        ...
        public ImportPermitActualAmendmentReportController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        public async Task<ActionResult<ApiResult<sp_ActualAmendReportResult>>> Post([FromBody] ImportPermitActualAmendmentReportRequest? request)
        {
            ...
            if (data.Count > 0)
            {
                result.CurrencyTotals = await ImportPermitListingCurrencyTotals.ExecuteAsync(
                    ...
                    procedureRequest.AmendRemarkId, formType: procedureRequest.FormType, sakhanId: 0);
            }
        ...
        public string ExcelWorksheetTitle => "Import Permit Actual Amendment Report";
        public Type ExcelRequestType => typeof(ImportPermitActualAmendmentReportRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((ImportPermitActualAmendmentReportRequest)request, sink, chunkSize, cancellationToken);
