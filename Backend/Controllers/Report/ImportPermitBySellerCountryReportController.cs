...
namespace Backend.Controllers.Report
{
    ...
    public class ImportPermitBySellerCountryReportController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "ImportPermitBySellerCountryReport";

        private readonly TradeNetDbContext _context;
        private readonly IExcelExportJobService _excelExportJobs;

        public ImportPermitBySellerCountryReportController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        [HttpPost]
        public async Task<ActionResult<ApiResult<ReportAggregateResult>>> Post([FromBody] ImportPermitBySellerCountryReportRequest? request)
        {
            ...
            var result = await sp_ImportPermitDetailReport_Fast.CreateAggregateResultAsync(
                ...
                // across currencies is meaningless (ImportPermitBySellerCountryReport.rdlc:839/893).
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] ImportPermitBySellerCountryReportRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Import Permit By Seller Country Report";
        public Type ExcelRequestType => typeof(ImportPermitBySellerCountryReportRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((ImportPermitBySellerCountryReportRequest)request, sink, chunkSize, cancellationToken);

        private async Task WriteRowsAsync(
            ImportPermitBySellerCountryReportRequest request,
            IExcelRowSink sink,
            int chunkSize,
            CancellationToken cancellationToken)
        {
            ...
            // CreatePagedResult -> Aggregate -> Order), so the exported rows appear in the order the
            // user saw on screen. Stated here at the append site because that guarantee must not
            // depend on the helper keeping its own internal ordering (the re-sort is idempotent).
    ...
    public sealed class ImportPermitBySellerCountryReportRequest : ReportQueryRequest
    {
