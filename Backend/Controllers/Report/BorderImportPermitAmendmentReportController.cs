...
namespace Backend.Controllers.Report
{
    ...
    public class BorderImportPermitAmendmentReportController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "BorderImportPermitAmendmentReport";
        ...
        public BorderImportPermitAmendmentReportController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        [HttpPost]
        public async Task<ActionResult<ApiResult<sp_AmendReportResult>>> Post([FromBody] BorderImportPermitAmendmentReportRequest? request)
        {
            ...
            if (data.Count > 0)
            {
                result.CurrencyTotals = await ImportPermitListingCurrencyTotals.ExecuteAsync(
                    _context, "Amend", procedureRequest!.FromDate, procedureRequest.ToDate,
                    procedureRequest.ExportImportSectionId, procedureRequest.CompanyRegistrationNo,
                    procedureRequest.AmendRemarkId, formType: procedureRequest.FormType, sakhanId: procedureRequest.SakhanId);
            }
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] BorderImportPermitAmendmentReportRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Border Import Permit Amendment Report";
        public Type ExcelRequestType => typeof(BorderImportPermitAmendmentReportRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((BorderImportPermitAmendmentReportRequest)request, sink, chunkSize, cancellationToken);

        private async Task WriteRowsAsync(
            BorderImportPermitAmendmentReportRequest request,
            IExcelRowSink sink,
            int chunkSize,
        ...
        private bool TryCreateReportRequest(
            BorderImportPermitAmendmentReportRequest? request,
            out sp_AmendReportRequest? procedureRequest,
    ...
    public sealed class BorderImportPermitAmendmentReportRequest : ReportQueryRequest
    {
