...
{
    /// <summary>
    /// Advance Search for Import Permit -- the old admin's Advance Search screen scoped to <c>Import Permit</c>
    ...
    public class AdvanceSearchImportPermitController : ControllerBase, IStreamingExcelReport
    {
        private const string ReportKey = "AdvanceSearchImportPermit";
        private const string LicenceOrPermitType = sp_AdvanceSearch.AdvanceSearchType.ImportPermit;

        private readonly TradeNetDbContext _context;
        private readonly IExcelExportJobService _excelExportJobs;

        public AdvanceSearchImportPermitController(TradeNetDbContext context, IExcelExportJobService excelExportJobs)
        {
            _context = context;
        ...
        public async Task<ActionResult<ApiResult<sp_AdvanceSearchResult>>> Post(
            [FromBody] AdvanceSearchImportPermitRequest? request)
        {
        ...
        [HttpPost("Excel")]
        public async Task<IActionResult> Excel([FromBody] AdvanceSearchImportPermitRequest? request)
        {
        ...
        public string ExcelWorksheetTitle => "Advance Search for Import Permit";
        public Type ExcelRequestType => typeof(AdvanceSearchImportPermitRequest);

        [NonAction]
        public Task WriteRowsAsync(object request, IExcelRowSink sink, int chunkSize, CancellationToken cancellationToken)
            => WriteRowsAsync((AdvanceSearchImportPermitRequest)request, sink, chunkSize, cancellationToken);

        private Task WriteRowsAsync(
            AdvanceSearchImportPermitRequest request,
            IExcelRowSink sink,
            int chunkSize,
    ...
    public sealed class AdvanceSearchImportPermitRequest : ReportQueryRequest
    {
