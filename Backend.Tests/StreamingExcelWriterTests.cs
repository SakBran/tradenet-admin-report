...
public sealed class StreamingExcelWriterTests
{
    ...
    // ---- The alternate-format members must be inert unless a layout asks for them ----

    [Fact]
    public void A_layout_that_does_not_set_the_alternate_format_members_writes_the_same_bytes()
    {
        // BlankRowsAfterHeader / SuppressStandardHeaderBlock / WorksheetTitle were added
        // for the DCCA export. Every other report leaves them at their defaults and must
        // keep receiving byte-identical files, or ~160 warm caches serve stale shapes.
        var baseline = WriteWithLayout([[AccountRow(1, 1000), AccountRow(2, 2500)]]);

        var untouched = AccountSummaryLayout();
        var explicitlyDefaulted = untouched.With(
            suppressStandardHeaderBlock: false,
            blankRowsAfterHeader: 0);

        Assert.Equal(
            baseline,
            WriteWithLayout([[AccountRow(1, 1000), AccountRow(2, 2500)]], explicitlyDefaulted));
    }

    [Fact]
    public void Blank_rows_after_the_header_push_the_data_down_and_repeat_on_every_sheet()
    {
        var layout = AccountSummaryLayout().With(blankRowsAfterHeader: 1);

        // maxRowsPerSheet is the test seam for rollover; 5 rows per sheet means the
        // preamble, header and spacer are re-emitted on sheet 2.
        using var ms = new MemoryStream();
        using (var writer = new StreamingExcelWriter(ms, "Account Summary Report", layout, 5))
        {
            writer.AppendRows(new[] { AccountRow(1, 100), AccountRow(2, 200), AccountRow(3, 300) });
            writer.Finish();
        }

        using var archive = new ZipArchive(new MemoryStream(ms.ToArray()), ZipArchiveMode.Read);

        foreach (var sheetIndex in Enumerable.Range(1, archive.Entries
            .Count(entry => entry.FullName.StartsWith("xl/worksheets/sheet", StringComparison.Ordinal))))
        {
            var sheet = ReadSheet(archive, sheetIndex);
            var ns = sheet.Root!.Name.Namespace;
            var rows = sheet.Descendants(ns + "row").ToList();

            // Title row, header row, then the spacer — cell-less, and never a data row.
            var spacer = rows[ExpectedPreambleRowCount + 1];
            Assert.Empty(spacer.Elements());

            AssertRowIndexesAreSane(sheet);
        }
    }

    /// <summary>Title lines + header block that <see cref="AccountSummaryLayout"/> emits.</summary>
    private const int ExpectedPreambleRowCount = 1;
}
