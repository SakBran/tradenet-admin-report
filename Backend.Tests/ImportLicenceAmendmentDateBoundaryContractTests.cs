...
public sealed class ImportLicenceAmendmentDateBoundaryContractTests
{
    ...
    public void Currency_totals_use_the_same_normalized_date_boundary_as_the_grid()
    {
        ...
        // At least the Amend and Actual Amendment branches; the substring also matches the
        // BorderImportLicence branches, which use the same boundary.
        Assert.True(CountOccurrences(source, normalizedBoundary) >= 2,
            "the Import Licence currency-totals branches must use the normalized boundary");
        Assert.DoesNotMatch(
    ...
    public void Import_licence_branch_normalizes_to_date_before_adding_one_day(string scriptName)
    {
        ...
        // At least the Import Licence branch's COUNT and page parts; the substring also matches
        // the BorderImportLicence branches, which use the same boundary.
        Assert.True(CountOccurrences(source, normalizedBoundary) >= 2,
            $"{scriptName} must normalize @ToDate in the Import Licence branch");
        Assert.DoesNotMatch(
            $@"(?<![A-Za-z]){System.Text.RegularExpressions.Regex.Escape(unnormalizedBoundary)}",
            source);
    }
