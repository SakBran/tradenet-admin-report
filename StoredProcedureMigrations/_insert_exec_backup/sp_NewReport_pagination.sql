...
BEGIN
    ...
    CREATE TABLE #r (
        ...
        [auto] nvarchar(max) NULL,
        [quota] nvarchar(max) NULL,
        [CommodityType] nvarchar(max) NULL,
        [__rn] int IDENTITY(1,1) NOT NULL
    );

    INSERT INTO #r ([Date], [SectionCode], [SectionName], [OldLicenceNo], [LicenceNo], [sDate], [CompanyRegistrationNo], [CompanyName], [UnitLevel], [StreetNumberStreetName], [QuarterCityTownship], [State], [Country], [PostalCode], [Currency], [Amount], [auto], [quota], [CommodityType])
    EXEC dbo.sp_NewReport @FormType, @FromDate, @ToDate, @ExportImportSectionId, @CompanyRegistrationNo, @SakhanId, @auto;
    ...
    DECLARE @valid bit = 0;
    IF @SortColumn IS NOT NULL AND @SortColumn IN (N'DateN','NSectionCodeN','NSectionNameN','NOldLicenceNoN','NLicenceNoN','NsDateN','NCompanyRegistrationNoN','NCompanyNameN','NUnitLevelN','NStreetNumberStreetNameN','NQuarterCityTownshipN','NStateN','NCountryN','NPostalCodeN','NCurrencyN','NAmountN','NautoN','NquotaN','NCommodityType') SET @valid = 1;

    DECLARE @orderby nvarchar(300);
    IF @valid = 1 SET @orderby = N'[' + @SortColumn + N'] ' + @dir + N', [__rn] ASC';
    ELSE SET @orderby = N'[__rn] ASC';

    DECLARE @sql nvarchar(max) = N'SELECT [Date], [SectionCode], [SectionName], [OldLicenceNo], [LicenceNo], [sDate], [CompanyRegistrationNo], [CompanyName], [UnitLevel], [StreetNumberStreetName], [QuarterCityTownship], [State], [Country], [PostalCode], [Currency], [Amount], [auto], [quota], [CommodityType], COUNT(*) OVER() AS TotalCount FROM #r ORDER BY ' + @orderby +
