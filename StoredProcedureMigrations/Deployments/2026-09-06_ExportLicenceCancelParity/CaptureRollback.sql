...
/* The legacy procedure the old Tradenet 2.0 admin still calls is the oracle for
   section 3 of VerifyDeployment.sql. Capture its text too: if production's copy has
   drifted from the 2022 snapshot this repository was built from, the parity diff has
   to be read against THIS text, not the repository's. */
