...
| [2026-09-14_BorderImportLicenceByHSCodeParity](2026-09-14_BorderImportLicenceByHSCodeParity/) | 1 procedure — `sp_HSCodeReport_pagination`: the `'Border Import Licence'` branch groups on `(HSCodeId, Currency)` like `BorderHSCodeReport.rdlc` (9,213 → 2,881 rows for 2025) with a unique page `ORDER BY`; supersedes the copy in `2026-09-08_ExportLicenceByHSCodeParity`. **Deploy before the application.** |
| [2026-09-17_MPUVoucherParity](2026-09-17_MPUVoucherParity/) | 2 procedures — the MPU reports' Voucher No. `sp_MPUReport_V3_pagination` stops numbering either side of its charge-to-voucher pairing over the report's own filtered rows (a row's voucher changed with the date range: 8 of 112 rows across two nested windows) and only considers rows Account Summary can display (`IsPayment = 1`); `sp_MPUReport_pagination` gains the same restriction on its four `TOP 1` subqueries. 5–7% of rows were showing a voucher from a different month. **Deploy before the application.** |
| [2026-09-25_CompanyProfileNewFormat](2026-09-25_CompanyProfileNewFormat/) | 1 procedure — `sp_CompanyProfileReport_pagination` returns `StartDate` and the capital's currency code and lists each company's directors in `SortOrder`, for the Company Profile layout the customer sends to the 11 ministries. **Deploy before the application**: the new app maps the two new columns and fails with HTTP 500 against the old procedure. |
