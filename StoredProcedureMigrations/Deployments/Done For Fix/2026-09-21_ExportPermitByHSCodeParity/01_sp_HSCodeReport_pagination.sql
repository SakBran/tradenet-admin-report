...
BEGIN
	...
	BEGIN
		...
		-- Import Permit / Export Licence / Border Import Licence branches: HSCodeReport.rdlc's row
		-- group is =Fields!HSCodeId.Value + =Fields!Currency.Value (rdlc:1152-1153) and the grid
		-- renders no company column, so the company in the key printed one HS code once per buyer
		-- -- visually identical rows each carrying a partial No of Licences / Total Value. Customer
		-- complaint 2026-09-21: 8807300000/USD showed 3 rows (1+4+1 licences) instead of one row of
		-- 6 / 12,950.0000; 2025 as a whole drops from 605 rows to 353. ExportPermitHSCodeDetailReport
		-- asks for the company shape explicitly with GroupBy='Company' and runs the LINQ twin
		-- instead. ORDER BY (HSCode, Currency, HSCodeId) is a unique key, so OFFSET/FETCH cannot
	...
	BEGIN
		-- Grouped on (HSCodeId, Currency) only -- NOT on the company, for the same reason as the
		-- Export Licence / Border Export Licence branches above: BorderHSCodeReport.rdlc's row
		-- group is =Fields!HSCodeId.Value + =Fields!Currency.Value (rdlc:1159-1162) and the grid
		-- has no company column, so the company in the key produced 9,213 rows where the old
		-- report shows 2,881 (2025) and 7,420 where it shows 2,690 (01/01-14/09/2026). Customer
		-- complaint 2026-09-14. BorderImportLicenceHSCodeDetailReport asks for the company shape
		-- explicitly with GroupBy='Company' and runs the LINQ twin instead. ORDER BY
		-- (HSCode, Currency, HSCodeId) is a unique key, so OFFSET/FETCH cannot repeat or drop a
		-- row across pages.
		IF(@HSCode='')
		BEGIN
			SELECT result.HSCode,result.HSDescription,
