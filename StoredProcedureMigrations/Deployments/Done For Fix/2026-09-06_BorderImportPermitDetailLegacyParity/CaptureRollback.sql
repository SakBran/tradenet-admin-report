/* =====================================================================================
   ...
   This release only CREATES dbo.sp_BorderImportPermitDetailReport_pagination, so on a first
   deployment the first query returns no row and the rollback is simply
       DROP PROCEDURE dbo.sp_BorderImportPermitDetailReport_pagination;
   (the application then falls back to its LINQ page on its own). If the procedure already
   exists, the row is its current CREATE text: to roll back, replace the leading CREATE with
   CREATE OR ALTER and execute the saved text.
...
SELECT OBJECT_DEFINITION(OBJECT_ID(N'dbo.sp_ImportPermitDetailReport')) AS legacy_sp_ImportPermitDetailReport;
GO
