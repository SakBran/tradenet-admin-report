...
// This report mirrors the legacy ImportLicenceByTotalValueLicenceReport.rdlc, which
// stacks TWO summary tables (Total Value per Currency, Total Licences per Pa Tha Ka
...
const config = reportConfigs.ImportLicenceTotalValueLicencesReport;
...
const ImportLicenceTotalValueLicencesReport = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      axiosInstance.get<LookupOption[]>('ReportLookups/importLicenceSections'),
    ])
  ...
  const heading = appliedRange
    ? `Import Licences Total Value & Licences (${appliedRange[0].format(
        'DD/MM/YYYY'
...
export default ImportLicenceTotalValueLicencesReport;
